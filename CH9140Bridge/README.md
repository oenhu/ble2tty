...
| 默认保存日志 | 自定义目录; 文件名模板({device}/{name}/{date}/{time}/{datetime}/{seq}); 一键切割新建文件; 按会话/按日期分目录/按日期合并三种方式, 跨午夜自动切换; 中文兼容(GBK 设备输出自动转 UTF-8) |
...
运行自检(85 项: 协议编解码/PTY 数据通路/日志/行装配/设置/MAC 解析):
